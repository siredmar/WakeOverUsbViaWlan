...
<div id=output></div><div id=systemsettings></div> \n		</td></tr></table></div></td></tr>"
	...
	$('#MainMenu > tbody:last-child').after( "\
		...
		<tr><td>PASS:</td><td><input type=\"password\" id=\"wificurpassword\"></td></tr> \
...
function SuspendToRamTypeSuspendCommand() 
{
      KeypressMulti("xfce4-session-logout -s");
      Keypress(40, 1, false); //Enter press
      Keypress(40, 0, false); //Enter release
}

function SuspendToRam()
{
   if(SuspendAllowed == 1)
   {
      i++;
      ...
      SuspendAllowed = 0;
      KeySetModifiers(4);
      Keypress(59, 1, false); //f2 press
      Keypress(59, 0, false); //f2 release
      KeySetModifiers(0);
      setTimeout(function(){SuspendToRamTypeSuspendCommand();}, 2000);      
   }
   else
   {
